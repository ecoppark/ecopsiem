...
docker build -t kaolat/sample-lab-kaolat:latest -f Dockerfile.dockerfile .
docker login
docker tag <local-image-id> kaolat/sample-lab-kaolat:latest
docker push kaolat/sample-lab-kaolat:latest